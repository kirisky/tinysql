...
func DecodeRecordKey(key kv.Key) (tableID int64, handle int64, err error) {
	...
	b, tID, err := codec.DecodeInt(key[len(tablePrefix):])
	b, rHandle, err := codec.DecodeInt(b[len(recordPrefixSep):])
	return tID, rHandle, err
}
...
func DecodeIndexKeyPrefix(key kv.Key) (tableID int64, indexID int64, indexValues []byte, err error) {
	...
	b, tableID, err := codec.DecodeInt(key[len(tablePrefix):])
	indexValues, indexID, err = codec.DecodeInt(b[len(indexPrefixSep):])
